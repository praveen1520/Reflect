...
const FolderTab = ({ colorClass }) => {
  return (
    <div
      className={`absolute inset-x-4 -top-2 h-2 rounded-t-md transform -skew-x-6 transition-colors ${colorClass} border`}
...
const CollectionsPreview = ({
  ...
  if (isCreateNew) {
    return (
      <button className="relative group h-[200px] w-full border bg-gray-50" onClick={onCreateNew} draggable="true">
        <FolderTab colorClass={colorSchemes["createCollection"].bg} />
        <div className={`relative h-full rounded p-6 shadow-md hover:shadow-lg transition-all flex flex-col items-center justify-content gap-4 ${colorSchemes["createCollection"].bg}`}>
  ...
  return (
   <Link href={`/collection/${isUnorganized ? "unorganied" : id}`} className="group relative h-[200px]"
    draggable="true" > <FolderTab
        colorClass={
          ...
          colorSchemes[isUnorganized ? "unorganized" : "collection"].bg
            ...
            <div className="space-y-2">
                ...
                <div className="space-y-2 mt-4 ">
                    {entries.length >0 ? (entries.slice(0,2).map((entry)=><EntryPreview key={entry.id} entry={entry}/>)):
